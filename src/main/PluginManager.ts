...
export class PluginManager {
    ...
    public async loadPlugin(pluginPath: string): Promise<string> {
        ...
        console.log(`workerEntry: ${workerEntry}, preloadPath: ${preloadPath}, rendererEntry: ${rendererEntry}`);
        console.log('------------------------------------------------');
        ...
        const worker = utilityProcess.fork(workerEntry, [], {
            ...
            stdio: 'pipe',
            env: { ...process.env, PLUGIN_ENV: 'production', PLUGIN_ID: pluginId, PLUGIN_RUN_MODE: 'worker' }
        });
